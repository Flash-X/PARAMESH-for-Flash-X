...
subroutine amr_restrict_pdgNo(mype,iopt,iempty,filling_guardcells,pdgNo)
  ...
  Use paramesh_dimensions, only: nfacevar,nvarcorn,nvaredge
...
#include "Flash_mpi_implicitNone.fh"
  ...
  if (npdg == -1) then
     ...
     write(*,*) 'entered amr_restrict_pdgNo'
...
! Perform restriction
  ...
  do ig = sg,eg
     ASSOCIATE(nxb         => gr_thePdgDimens(ig) % nxb,      &
       ...
       if(nfacevar.gt.0.and.iopt.eq.1.and.ig.eq.1) lfc = .true.
       if(nvaredge.gt.0.and.iopt.eq.1.and.ig.eq.1) lec = .true.
...
#ifdef DEBUG_FLOW_TRACE
      write(*,*) 'exiting amr_restrict_pdgNo'
#endif /* DEBUG_FLOW_TRACE */
